...
- [x] (2026-04-25 00:37:29Z) Milestone 5c: `src/ui/file_view.rs` を追加し、file-view renderer と file-view line renderer を `src/ui.rs` から分離した。
- [x] (2026-04-25 00:38:33Z) file-view renderer 抽出後の full gate `just ci` を実行し、fmt-check、clippy、467 unit tests、release build、e2e 35 件成功を確認した。
- [x] (2026-04-25 00:49:31Z) Milestone 5d: `src/ui/diff_line.rs` と `src/ui/text_cells.rs` を追加し、diff-line renderer、search highlight overlay、cell-width wrapping helper を `src/ui.rs` から分離した。
- [x] (2026-04-25 00:50:39Z) diff-line/text cell 抽出後の full gate `just ci` を実行し、fmt-check、clippy、467 unit tests、release build、e2e 35 件成功を確認した。
- [x] (2026-04-25 00:55:42Z) Milestone 5e: `src/ui/diff_view.rs` を追加し、scroll-view renderer、row renderer、sticky header、cursor gutter tint、file/hunk header rendering を `src/ui.rs` から分離した。
- [x] (2026-04-25 00:57:03Z) diff-view renderer 抽出後の full gate `just ci` を実行し、fmt-check、clippy、467 unit tests、release build、e2e 35 件成功を確認した。
- [x] (2026-04-25 01:02:44Z) Milestone 5f: `src/ui/frame.rs`、`src/ui/input_line.rs`、`src/ui/empty_state.rs`、`src/ui/tests.rs` を追加し、top-level frame composition、input-line renderer、empty state、UI tests を `src/ui.rs` から分離した。
- [x] (2026-04-25 01:02:44Z) UI facade 仕上げ後の full gate `just ci` を実行し、fmt-check、clippy、467 unit tests、release build、e2e 35 件成功を確認した。
- [x] (2026-04-25 01:06:37Z) Milestone 6a: `src/git/types.rs`、`src/git/repo.rs`、`src/git/revert.rs` を追加し、diff model 型、repo identity command、hunk revert を `src/git.rs` から分離した。
- [x] (2026-04-25 01:12:44Z) Milestone 6b: `src/git/parse.rs`、`src/git/path.rs`、`src/git/untracked.rs`、`src/git/diff.rs` を追加し、parser、path byte conversion、untracked synth、diff command / snapshot 分割を `src/git.rs` から分離した。
- [x] (2026-04-25 01:12:44Z) git module 分割後の full gate `just ci` を実行し、fmt-check、clippy、467 unit tests、release build、e2e 35 件成功を確認した。
- [x] (2026-04-25 01:18:53Z) Milestone 7a: `src/init/types.rs`、`src/init/detect.rs`、`src/init/settings_json.rs`、`src/init/tests.rs` を追加し、agent model / detection、settings JSON hook merge/removal、init tests を `src/init.rs` から分離した。
- [x] (2026-04-25 01:18:53Z) init 第一波分割後の full gate `just ci` を実行し、fmt-check、clippy、467 unit tests、release build、e2e 35 件成功を確認した。
- [x] (2026-04-25 01:22:25Z) Milestone 7b: `src/init/pre_commit.rs` を追加し、pre-commit shim install/removal と shell quoting helper を `src/init.rs` から分離した。
- [x] (2026-04-25 01:22:25Z) pre-commit shim 分割後の full gate `just ci` を実行し、fmt-check、clippy、467 unit tests、release build、e2e 35 件成功を確認した。
- [x] (2026-04-25 01:26:35Z) Milestone 7c: `src/init/teardown.rs` を追加し、teardown/reporting を `src/init.rs` から分離した。
- [x] (2026-04-25 01:26:35Z) teardown 分割後の full gate `just ci` を実行し、fmt-check、clippy、467 unit tests、release build、e2e 35 件成功を確認した。
- [x] (2026-04-25 01:31:48Z) Milestone 7d: `src/init/install.rs` を追加し、scope fallback、hook command construction、agent-specific installer dispatch を `src/init.rs` から分離した。
- [x] (2026-04-25 01:31:48Z) installer dispatch 分割後の検証として `cargo test --all-targets --all-features init::tests -- --nocapture` 18 件、`cargo clippy --all-targets --all-features -- -D warnings`、full gate `just ci` を実行し、unit tests 467 件、e2e 35 件成功を確認した。
- [x] (2026-04-25 01:31:48Z) 追加 perf probe は今回は不要と判断した。hot path の新規アルゴリズム変更ではなく責務分割であり、app/ui/git/init の各波で full e2e を通して外部挙動を pin した。
- [x] (2026-04-25 01:36:53Z) Milestone 8: `src/hook.rs` を facade 化し、types、input parser、scar scan、output formatting、event log、session file enumeration、tests を `src/hook/` 配下に分離した。
- [x] (2026-04-25 01:36:53Z) hook 分割後の検証として `cargo test --all-targets --all-features hook::tests -- --nocapture` 30 件、`cargo clippy --all-targets --all-features -- -D warnings`、full gate `just ci` を実行し、unit tests 467 件、e2e 35 件成功を確認した。
- [x] (2026-04-25 01:42:09Z) Milestone 9: `src/watcher.rs` を facade 化し、backend selection、baseline matcher、worktree watcher、git state watcher、event-log watcher、tests を `src/watcher/` 配下に分離した。
- [x] (2026-04-25 01:42:09Z) watcher 分割後の検証として `cargo test --all-targets --all-features watcher::tests -- --nocapture` 16 件、`cargo clippy --all-targets --all-features -- -D warnings`、full gate `just ci` を実行し、unit tests 467 件、e2e 35 件成功を確認した。
...
- Observation: file-view renderer は `cursor_bar`、`wrap_at_chars`、`take_cells`、`eof_no_newline_span` を parent module から借りるだけで分離できた。
  Evidence: `cargo test --all-targets --all-features file_view -- --nocapture` は 16 件成功し、`cargo test --all-targets --all-features ui::tests -- --nocapture` は 63 件成功した。`cargo clippy --all-targets --all-features -- -D warnings` も成功した。

- Observation: file-view renderer 抽出後も full gate を通過した。
  Evidence: `just ci` は `cargo fmt --all -- --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test --all-targets --all-features`、`cargo build --release --locked`、`bun test` を順に実行し、unit tests は `467 passed`、e2e は `35 pass / 0 fail` だった。

- Observation: diff-line renderer は `cursor_bar` だけを parent module から借り、cell-width wrapping と EOF marker を `text_cells.rs` に分離できた。
  Evidence: `cargo test --all-targets --all-features render_diff_line -- --nocapture` は 5 件成功し、`cargo test --all-targets --all-features wrap -- --nocapture` は 31 件成功し、`cargo test --all-targets --all-features search -- --nocapture` は 19 件成功した。`cargo clippy --all-targets --all-features -- -D warnings` も成功した。

- Observation: diff-line/text cell 抽出後も full gate を通過した。
  Evidence: `just ci` は `cargo fmt --all -- --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test --all-targets --all-features`、`cargo build --release --locked`、`bun test` を順に実行し、unit tests は `467 passed`、e2e は `35 pass / 0 fail` だった。

- Observation: scroll-view renderer は `render()` 入口を変えずに `diff_view.rs` へ移動できたが、`cursor_bar` と `format_mtime` は diff/file/overlay の共有点として parent `ui.rs` に残す必要があった。
  Evidence: 最初の移動後に `super::cursor_bar` と `super::format_mtime` の未解決 import が出たため、共有関数だけ `ui.rs` に戻した。修正後、`cargo test --all-targets --all-features ui::tests -- --nocapture` は 63 件成功し、`cargo clippy --all-targets --all-features -- -D warnings` も成功した。

- Observation: diff-view renderer 抽出後も full gate を通過した。
  Evidence: `just ci` は `cargo fmt --all -- --check`、`cargo clippy --all-targets --all-features -- -D warnings`、`cargo test --all-targets --all-features`、`cargo build --release --locked`、`bun test` を順に実行し、unit tests は `467 passed`、e2e は `35 pass / 0 fail` だった。

- Observation: UI facade の仕上げで production entry point は `src/ui/frame.rs` へ移り、`src/ui.rs` は module 宣言、共有 helper、test module declaration だけに近づいた。
  Evidence: `wc -l src/ui.rs src/ui/*.rs` は `92 src/ui.rs`、`71 src/ui/frame.rs`、`92 src/ui/input_line.rs`、`28 src/ui/empty_state.rs`、`1793 src/ui/tests.rs` を表示した。`cargo test --all-targets --all-features ui::tests -- --nocapture` は 63 件成功し、`just ci` も unit tests `467 passed`、e2e `35 pass / 0 fail` で成功した。

- Observation: git 第一波は public API を re-export で維持したまま、型・repo command・hunk revert の責務だけを分離できた。
  Evidence: `wc -l src/git.rs src/git/*.rs` は `1882 src/git.rs`、`110 src/git/types.rs`、`136 src/git/repo.rs`、`66 src/git/revert.rs` を表示した。`cargo test --all-targets --all-features git::tests -- --nocapture` は 42 件成功し、`cargo test --all-targets --all-features stream -- --nocapture` は 7 件成功し、`cargo clippy --all-targets --all-features -- -D warnings` も成功した。

- Observation: git 第二波で production backend は child modules に移り、`src/git.rs` は facade と legacy tests の holder になった。
  Evidence: `wc -l src/git.rs src/git/*.rs` は `1147 src/git.rs`、`200 src/git/diff.rs`、`321 src/git/parse.rs`、`231 src/git/untracked.rs`、`17 src/git/path.rs`、`136 src/git/repo.rs`、`66 src/git/revert.rs`、`110 src/git/types.rs` を表示した。`cargo test --all-targets --all-features git::tests -- --nocapture` は 42 件成功し、`cargo test --all-targets --all-features stream -- --nocapture` は 7 件成功し、`just ci` は unit tests `467 passed`、e2e `35 pass / 0 fail` で成功した。

- Observation: init 第一波では agent model / detection と settings JSON schema 操作を切り出し、`src/init.rs` を 938 行まで縮小できた。
  Evidence: `wc -l src/init.rs src/init/*.rs` は `938 src/init.rs`、`170 src/init/types.rs`、`28 src/init/detect.rs`、`329 src/init/settings_json.rs`、`658 src/init/tests.rs` を表示した。`cargo test --all-targets --all-features init::tests -- --nocapture` は 18 件成功し、`just ci` は unit tests `467 passed`、e2e `35 pass / 0 fail` で成功した。

- Observation: pre-commit shim は install と teardown の両方から参照される独立責務として `pre_commit.rs` に分離できた。
  Evidence: `wc -l src/init.rs src/init/*.rs` は `813 src/init.rs`、`129 src/init/pre_commit.rs`、`329 src/init/settings_json.rs`、`170 src/init/types.rs`、`28 src/init/detect.rs`、`658 src/init/tests.rs` を表示した。`cargo test --all-targets --all-features init::tests -- --nocapture` は 18 件成功し、`just ci` は unit tests `467 passed`、e2e `35 pass / 0 fail` で成功した。

- Observation: teardown/reporting は `pre_commit` と `settings_json` の削除 helper を呼ぶ coordinator として分離できた。
  Evidence: `wc -l src/init.rs src/init/*.rs` は `659 src/init.rs`、`156 src/init/teardown.rs`、`129 src/init/pre_commit.rs`、`329 src/init/settings_json.rs`、`170 src/init/types.rs`、`28 src/init/detect.rs`、`658 src/init/tests.rs` を表示した。`cargo test --all-targets --all-features init::tests -- --nocapture` は 18 件成功し、`just ci` は unit tests `467 passed`、e2e `35 pass / 0 fail` で成功した。

- Observation: installer dispatch を最後に切り出したことで、`src/init.rs` は run_init と prompt/reporting の facade に近づき、agent-specific 書き込み責務は `install.rs` にまとまった。
  Evidence: `wc -l src/init.rs src/init/*.rs` は `292 src/init.rs`、`378 src/init/install.rs`、`154 src/init/teardown.rs`、`130 src/init/pre_commit.rs`、`329 src/init/settings_json.rs`、`170 src/init/types.rs`、`28 src/init/detect.rs`、`658 src/init/tests.rs` を表示した。`cargo test --all-targets --all-features init::tests -- --nocapture` は 18 件成功し、`cargo clippy --all-targets --all-features -- -D warnings` と `just ci` も成功した。

- Observation: hook module は v0.2/v0.3 の境界に沿って自然に分割できた。hook facade は public API re-export だけを持ち、event log の test-only prune helper は `cfg(test)` re-export に絞った。
  Evidence: `wc -l src/hook.rs src/hook/*.rs` は `16 src/hook.rs`、`163 src/hook/event_log.rs`、`72 src/hook/input.rs`、`57 src/hook/output.rs`、`109 src/hook/scan.rs`、`118 src/hook/session_files.rs`、`61 src/hook/types.rs`、`583 src/hook/tests.rs` を表示した。`cargo test --all-targets --all-features hook::tests -- --nocapture` は 30 件成功し、`just ci` は unit tests `467 passed`、e2e `35 pass / 0 fail` で成功した。

- Observation: watcher module は notify backend の OS 差分、baseline matcher、worktree event filtering、git state roots、stream event-log watch をそれぞれ別 module に切り出せた。`watcher::start`、`WatchHandle`、`WatchEvent` は既存 API のまま facade に残した。
  Evidence: `wc -l src/watcher.rs src/watcher/*.rs` は `227 src/watcher.rs`、`73 src/watcher/backend.rs`、`63 src/watcher/event_log.rs`、`111 src/watcher/git_state.rs`、`95 src/watcher/matcher.rs`、`145 src/watcher/worktree.rs`、`770 src/watcher/tests.rs` を表示した。`cargo test --all-targets --all-features watcher::tests -- --nocapture` は 16 件成功し、`just ci` は unit tests `467 passed`、e2e `35 pass / 0 fail` で成功した。
...
今回の大規模責務分割では、`src/app.rs`、`src/ui.rs`、`src/git.rs`、`src/init.rs`、`src/hook.rs`、`src/watcher.rs` の巨大ファイルを facade と責務別 child module 群に分解した。`App` の layout/navigation/input/review/runtime、UI の geometry/diff/file/frame/input-line、git の types/repo/diff/parse/untracked/revert、init の types/detect/settings/pre-commit/teardown/install、hook の input/scan/output/event-log/session-file、watcher の backend/matcher/worktree/git-state/event-log がそれぞれ読み始められる単位になった。

ユーザーから見える挙動は変えていない。各波で targeted tests を通し、主要コミットごとに `just ci` が完走した。最終状態でも fmt-check、clippy、467 unit tests、release build、35 e2e tests がすべて成功した。残るリスクは分割後に責務境界をまたぐ小さな helper が増えすぎることなので、次の機能追加では facade に戻さず、該当 module へ直接テストと実装を置くのがよい。
...
`src/ui/file_view.rs` は `render_file_view`、`render_file_view_line`、`render_file_view_line_wrapped` を持つ。`render_file_view` は `RenderGeometry::for_file_view` を使い、line-number span insertion は `line_numbers.rs` を通す。

`src/ui/diff_line.rs` は `render_diff_line`、`render_diff_line_wrapped` と search highlight overlay を持つ。`src/ui/text_cells.rs` は `wrap_at_chars`、`take_cells`、`eof_no_newline_span` を持ち、diff view と file view の cell-width aware rendering で共有する。

`src/ui/diff_view.rs` は `render_scroll`、row renderer、sticky header、cursor gutter tint、file/hunk header rendering を持つ。`src/ui.rs` は top-level `render`、input line、empty state、共有 `cursor_bar` / `format_mtime` を残す。
