...
今回の大規模責務分割では、`src/app.rs`、`src/ui.rs`、`src/git.rs`、`src/init.rs` の巨大ファイルを facade と責務別 child module 群に分解した。`App` の layout/navigation/input/review/runtime、UI の geometry/diff/file/frame/input-line、git の types/repo/diff/parse/untracked/revert、init の types/detect/settings/pre-commit/teardown/install がそれぞれ読み始められる単位になった。
