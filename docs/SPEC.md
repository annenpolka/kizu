...
- **シンタックスハイライト**: syntect（bat/deltaと同じエンジン）を既定にし、JS/TS/JSX/TSX は tree-sitter による文書単位 highlight を使う
...
| .jsx, .tsx | 文脈依存。JavaScript / TypeScript の statement では `// @kizu[<kind>]: ...`、JSX children / fragment / element では `{/* @kizu[<kind>]: ... */}`。複数行 props の途中では JSX element の直上へ `{/* ... */}` を移して挿入 |
